...
def extract_company_embeddings(company_id):
    ...
    for job in company.get("jobs", []):
        ...
        job_description_embedding = (
            get_embedding(job_description) if job_description else None
        )
...
def recommend_companies(user_id):
    ...
    resume_embeddings = [
        extract_resume_embeddings(resume) for resume in resumes if resume
    ]
    ...
    mean_summary_embedding = [
        emb for emb in resume_summary_embeddings if emb is not None
    ]

    if not mean_skill_embedding and not mean_summary_embedding:
        print(
            f"No valid user embeddings to combine for user: {user_id}", file=sys.stderr
        )
        return []

    user_average_skill_embedding = (
        np.mean(np.array(mean_skill_embedding), axis=0)
        if mean_skill_embedding
        else None
    )
    user_average_summary_embedding = (
        np.mean(np.array(mean_summary_embedding), axis=0)
        if mean_summary_embedding
        else None
    )

    embeddings_to_combine = [
        emb
        for emb in [user_average_skill_embedding, user_average_summary_embedding]
        if emb is not None
    ]
    ...
    companies_in_industry = (
        db.companies.find({"industry": user.get("industry")})
        if user.get("industry")
        else db.companies.find({})
    )
    ...
    for company in companies_in_industry:
        ...
        (
            ...
            job_title_embeddings,
            job_description_embeddings,
        ) = extract_company_embeddings(company_id)

        if company_description_embedding is None:
            continue

        company_all_embeddings = (
            [company_description_embedding]
            + job_skill_embeddings
            + job_title_embeddings
            + job_description_embeddings
        ...
        company_all_embeddings = [
            emb for emb in company_all_embeddings if emb is not None
        ]
