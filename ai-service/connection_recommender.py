...
def recommend_connections(user_resume):
    ...
    for resume in all_resumes:
        mean_skill_embedding, certification_embeddings = extract_resume_embeddings(
            resume
        )
        ...
        if embeddings:
            ...
            mean_skill_embedding = (
                mean_skill_embedding.flatten()
                if mean_skill_embedding is not None
                else torch.tensor([])
            )
            certification_embeddings = (
                certification_embeddings.flatten()
                if certification_embeddings is not None
                else torch.tensor([])
            )
            ...
            skill_similarity = (
                torch.cosine_similarity(user_skill_embedding, mean_skill_embedding)
                if mean_skill_embedding is not None
                else torch.tensor(0.0)
            )
            certification_similarity = (
                torch.cosine_similarity(
                    user_certification_embedding, certification_embeddings
                )
                if certification_embeddings is not None
                else torch.tensor(0.0)
            )
