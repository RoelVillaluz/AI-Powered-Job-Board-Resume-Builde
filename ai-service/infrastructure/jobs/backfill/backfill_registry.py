...
BACKFILL_REGISTRY: dict[str, BackfillConfig] = {
    "skills": {
        ...
        "validator": lambda data: (
            bool(data.get("skill_ids"))
            and bool(data.get("skill_embeddings"))
            and len(data["skill_ids"]) == len(data["skill_embeddings"])
        ),
    ...
    "job_title": {
        ...
        "validator": lambda data: bool(
            data.get("title_id") and data.get("title_embedding") is not None
        ),
    ...
    "location": {
        ...
        "validator": lambda data: bool(
            data.get("location_id") and data.get("location_embedding") is not None
        ),
