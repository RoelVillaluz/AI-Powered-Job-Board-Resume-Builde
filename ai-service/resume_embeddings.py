    ...
    try:
        ...
        if not force_regenerate:
            ...
            if existing:
                ...
                if (
                    existing.get("meanEmbeddings", {}).get("skills")
                    and len(existing.get("meanEmbeddings", {}).get("skills", [])) > 0
                ):
                    return {
                        "cached": True,
                        "embeddings": {
                            "skills": existing.get("embeddings", {}).get("skills", []),
                            "workExperience": existing.get("embeddings", {}).get(
                                "workExperience", []
                            ),
                            "certifications": existing.get("embeddings", {}).get(
                                "certifications", []
                            ),
                        },
                        "meanEmbeddings": {
                            "skills": existing.get("meanEmbeddings", {}).get(
                                "skills", []
                            ),
                            "workExperience": existing.get("meanEmbeddings", {}).get(
                                "workExperience"
                            ),
                            "certifications": existing.get("meanEmbeddings", {}).get(
                                "certifications"
                            ),
                        },
                        "metrics": {
                            "totalExperienceYears": existing.get("metrics", {}).get(
        ...
        mean_skill, mean_work, mean_cert, total_years = extract_resume_embeddings(
            resume
        )
        ...
        result = {
            ...
            "metrics": {"totalExperienceYears": float(total_years)},
        }
