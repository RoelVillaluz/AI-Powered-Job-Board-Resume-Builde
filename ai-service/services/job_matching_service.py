...
class JobMatchingService:
    ...
    def score_match(
        ...
        required_penalty = min(len(missing_required) * PENALTY_MISSING_REQUIRED_SKILL, 40.0)
    ...
    def _skill_match(
        ...
        for skill in all_job_skills:
            ...
            if market:
                ...
                market_value = ((demand_score * 0.6) + (growth_rate * 0.4)) * seniority_m
    ...
    def _experience_fit(candidate_years: float, metadata: dict) -> float:
        ...
        if gap <= 1:   return 85.0
        if gap <= 2:   return 70.0
        if gap <= 3:   return 55.0
    ...
    def _cert_bonus(resume: dict, metadata: dict) -> float:
        resume_certs   = {c.get("name", "").lower() for c in resume.get("certifications", [])}
    ...
    def _seniority_gap(resume: dict, metadata: dict) -> int:
        ...
        try:
            return abs(
                SENIORITY_LADDER.index(resume_level) -
                SENIORITY_LADDER.index(job_level)
            )
        except ValueError:
            return 0
    ...
    def _insights(
        ...
        resume_cert_names = {c.get("name", "").lower() for c in resume.get("certifications", [])}
        required_certs    = metadata.get("requiredCertifications", [])
        if required_certs and any(c.lower() in resume_cert_names for c in required_certs):
        ...
        if required_certs and not any(c.lower() in resume_cert_names for c in required_certs):
