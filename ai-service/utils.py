...
def get_user_interacted_jobs(user_id):
    ...
    try:
        ...
        if user_data:
            ...
            interacted_jobs["applied_jobs"] = [
                application["jobPosting"] for application in applied_jobs
            ]
        else:
...
def extract_resume_embeddings(resume):
    ...
    certifications = [
        certification["name"] for certification in resume.get("certifications", [])
    ]

    # Compute embeddings safely
    skill_embeddings = [
        get_embedding(skill) for skill in skills if get_embedding(skill) is not None
    ]
    work_embeddings = [
        get_embedding(exp) for exp in work_experiences if get_embedding(exp) is not None
    ]
    certification_embeddings = [
        get_embedding(cert)
        for cert in certifications
        if get_embedding(cert) is not None
    ]
    ...
    if certification_embeddings:
        ...
        certification_embeddings = (
            torch.mean(certification_embeddings_tensor, dim=0).detach().cpu()
        )
    else:
    ...
    mean_skill_embedding = (
        torch.mean(skill_embeddings, dim=0).detach().cpu()
        if skill_embeddings is not None
        else None
    )
    mean_work_embedding = (
        torch.mean(work_embeddings, dim=0).detach().cpu()
        if work_embeddings is not None
        else None
    )
...
def extract_job_embeddings(job):
    ...
    skill_embeddings = (
        torch.stack([get_embedding(skill) for skill in skills]) if skills else None
    )
    requirement_embeddings = (
        torch.stack([get_embedding(req) for req in requirements])
        if requirements
        else None
    )
    ...
    experience_embedding = (
        get_embedding(job.get("experienceLevel", ""))
    ...
    job_title_embedding = (
        get_embedding(job.get("title", "")) if job.get("title") else None
    )
    if job_title_embedding is not None:
        job_title_embedding = job_title_embedding.detach().cpu()

    location_embedding = (
        get_embedding(job.get("location", "")) if job.get("location") else None
    )
    ...
    if requirement_embeddings is not None and requirement_embeddings.numel() > 0:
        mean_requirements_embedding = (
            torch.mean(requirement_embeddings, dim=0).detach().cpu()
        )
