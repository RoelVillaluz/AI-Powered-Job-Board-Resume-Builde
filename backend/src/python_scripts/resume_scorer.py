...
import torch
from backend.src.python_scripts.utils import extract_job_embeddings, extract_resume_embeddings, get_embedding, get_resume_by_id
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) < 3:
            print(json.dumps({"error": "Missing resumeId argument"}))
            sys.exit(1)
        ...
        if not resume:
            print(json.dumps({"error": f"Resume not found with ID: {resume_id}"}))
            sys.exit(1)
