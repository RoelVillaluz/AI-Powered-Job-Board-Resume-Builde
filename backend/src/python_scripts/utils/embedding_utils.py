...
def extract_skills_embeddings(
    ...
    Returns:
        Tuple of:
          ...
          - list of skill IDs needing backfill (exist in DB but embedding is null)
          - list of per-skill fallback embeddings aligned to backfill IDs,
            so the caller can write the correct individual embedding per skill
            rather than the aggregated mean
    """
    ...
    missing_skills: list[str] = []    # names that need model fallback
    needs_backfill: list[str] = []    # IDs that exist in DB but have null embedding
 
    for skill_name in skill_names:
        skill_doc = skill_map.get(skill_name)
 
        if skill_doc and skill_doc.get('embedding'):
            all_embeddings.append(torch.tensor(skill_doc['embedding']))
            ...
            missing_skills.append(skill_name)
            # not in DB — no ID to backfill, but still needs encoding
    ...
    if missing_skills:
        ...
        if fallback is not None:
            ...
            # Only the first len(needs_backfill) fallbacks correspond to DB entries
            # with null embeddings — the rest are skills not in the DB at all.
            # missing_skills is ordered: null-embedding skills come before
            # not-in-DB skills because the loop appends them in that order.
