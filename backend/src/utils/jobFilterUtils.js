...
export const buildJobQuery = (filters) => {
    ...
    // Industry filter (handled via company population, see below)
    // We'll need to handle this differently with $lookup or post-filter
...
export const buildSortQuery = (sortBy) => {
    switch (sortBy) {
        ...
        case 'Newest First':
            return { postedAt: -1 };
        case 'Highest Salary':
            return { 'salary.amount': -1 };
        case 'Best Match':
        default:
            return { postedAt: -1 }; // Default sort by newest
    }
};
...
export const parseFilterParams = (queryParams) => {
    ...
    // Pagination
    filters.skip = Number(queryParams.skip) || 0;
